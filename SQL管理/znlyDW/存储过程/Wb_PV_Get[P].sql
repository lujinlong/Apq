...
DECLARE @rtn int,@ExMsg nvarchar(max),@Imei nvarchar(50)
	,@FirstTime datetime,@FirstLogType int,@FirstPlatform nvarchar(100),@FirstSMSC nvarchar(50),@FirstProvince nvarchar(50)
	,@LastTime datetime,@LastLogType int,@LastPlatform nvarchar(100),@LastSMSC nvarchar(50),@LastProvince nvarchar(50)
...
EXEC @rtn = dbo.Wb_PV_Get @ExMsg out, @Imei
	...
	;
SELECT @rtn;
SELECT @FirstTime,@FirstLogType,@FirstPlatform,@FirstSMSC,@FirstProvince
	,@LastTime,@LastLogType,@LastPlatform,@LastSMSC,@LastProvince
	,@VisitCountTotal,@VisitCountWeek,@VisitCountDWeek,@VisitCountMonth,@VisitCountNMonth
-- =============================================
*/
SET NOCOUNT ON;

SELECT @FirstTime = FirstTime,@FirstLogType=FirstLogType,@FirstPlatform=FirstPlatform,@FirstSMSC=FirstSMSC,@FirstProvince=FirstProvince
	,@LastTime=LastTime,@LastLogType=LastLogType,@LastPlatform=LastPlatform,@LastSMSC=LastSMSC,@LastProvince=LastProvince
