IF( OBJECT_ID('dbo.GameWS_ChargeLog_Add_R2', 'P') IS NULL )
	EXEC sp_executesql N'CREATE PROCEDURE dbo.GameWS_ChargeLog_Add_R2 AS BEGIN RETURN END';
GO
ALTER PROC dbo.GameWS_ChargeLog_Add_R2
...
-- 示例:
DECLARE @rtn int;
EXEC @rtn = dbo.GameWS_ChargeLog_Add_R2;
SELECT @rtn;
-- =============================================
*/
SET NOCOUNT ON;
...
BEGIN
	-- 达到月限时将日统计行对应列置为1
	UPDATE dbo.UserChannelPay_Today
	   SET [_Time] = getdate(), IsMonthLimit = 1
	 WHERE PayWay = @PayWay AND uid = @uid;
END
*/
...
RETURN 1;
GO
GRANT EXECUTE ON [dbo].[GameWS_ChargeLog_Add_R2] TO GameW
GO
