<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Login.aspx.cs" Inherits="Dinner.Admin.Login" %>
	...
	<script type="text/javascript">
		...
		function btnLogin_Click() {
			...
			if (strLoginName && strLoginPwd) {
				...
				Dinner.WS.WS2.Login_LoginName(strLoginName, strLoginPwd, Login_Success, Apq_WS_Faild);
			}
		}
		...
		function Login_Success(stReturn) {
			...
			if (stReturn.NReturn != 1) {
				Ext.Msg.alert("登录错误", stReturn.ExMsg);
				return;
			}

			// Cookie 操作
			Ext.state.Cookies.set("LoginName", stReturn.FNReturn.rows[0]["LoginName"]);
			Ext.state.Cookies.set("SqlLoginPwd", stReturn.POuts[0]);

			top.location = stReturn.FNReturn.rows[0]["IsAdmin"] ? "Main.aspx" : "../User/Main.aspx";
